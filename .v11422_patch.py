...
a=s.index('async function ensureSimpleMode(page){')
z=s.index('\nasync function resetCart(page){',a)
replacement=r'''async function ensureSimpleMode(page){
...
a=s.index('async function cartStakeInputIds(page){')
z=s.index('\nasync function stakeInputForBet(page,b,index=null){',a)
replacement=r'''async function cartStakeInputIds(page){
...
a=s.index('async function stakeInputForBet(page,b,index=null){')
...
old="""    stage='binding final des mises';
...
"""
new="""    stage='ouverture panier Simple';
...
s=s.replace('11.4.21','11.4.22')
p.write_text(s)

p=Path('index.html')
p.write_text(p.read_text().replace('V11.4.21','V11.4.22'))
p=Path('package.json')
p.write_text(p.read_text().replace('"version":"11.4.21"','"version":"11.4.22"'))
