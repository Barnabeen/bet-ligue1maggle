...
a=s.index('async function scrapeLigue1Maggle(page){')
z=s.index('\nconst PS_L1_URL =',a)
replacement=r'''async function scrapeLigue1Maggle(page,targetEvents=[]){
...
if old not in s: raise SystemExit('sync block not found')
s=s.replace(old,new,1)

s=s.replace('11.4.24','11.4.25')
p.write_text(s)
p=Path('index.html'); p.write_text(p.read_text().replace('V11.4.24','V11.4.25'))
p=Path('package.json'); p.write_text(p.read_text().replace('"version":"11.4.24"','"version":"11.4.25"'))
